...
class WeatherService {
    ...
    private func currentTemperature(latitude: Double, longitude: Double, _ response: (Float, String) -> Void) async {
        ...
        do {
            ...
            os_log("Received data: %@", log: self.weatherLog, String(decoding: data, as: UTF8.self))
            let jsonObject = try JSONSerialization.jsonObject(with: data)
            guard let json = jsonObject as? [String: Any] else {
                os_log("Failed to decode JSON", log: self.weatherLog)
                return
            }
            os_log("Encoded keys: %@", log: self.weatherLog, json.keys.map(\.description).joined(separator: ", "))
    ...
    func currentTemperature(city: String, _ response: (Float, String) -> Void) async {
        do {
            ...
            await currentTemperature(latitude: latD, longitude: lonD, response)
